<?php
    ...
    if(isset($postdata) && !empty($postdata)){
        ...
        try{
            ...
            $stmt = $pdo->prepare(Produit::get_insert_query_prepared());
            $stmt->execute($produit->get_values_prepared());

            $stmt_counter = $pdo->prepare(Produit::get_update_counter_query_prepared());
            $stmt_counter->execute([]);

            $pdo->commit();
            print json_encode(["status"=>"success", "message"=>"Produit enregistrée avec succès !"]);
        } catch (Exception $e) {
            $pdo->rollBack();
