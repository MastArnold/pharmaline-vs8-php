<?php
    ...
    if(isset($postdata) && !empty($postdata)){
        ...
        try{
            ...
            $stmt = $pdo->prepare(Client::get_update_query_prepared());
            $stmt->execute($client->get_update_values_prepared());
        
            $pdo->commit();            
            print json_encode(["status"=>"success", "message"=>"Client mise à jour avec succès !"]);
        } catch (Exception $e) {
            $pdo->rollBack();
